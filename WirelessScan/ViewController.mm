...
@interface ViewController () {
    ...
    BOOL m_bLoadedImage;        //  有图像的情况下也分为两种情况，一种是重新载入的图像（1），一种是重建的图像（0）（回放或者在运行中）
...
- (void)viewDidLoad {
    ...
    //
    {
        ...
        UIImage *resultImage=UIGraphicsGetImageFromCurrentImageContext();  //图片缩放
        ...
        UITapGestureRecognizer* tap = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(tapInImageV:)];   //点击手势
        [self.scanImg addGestureRecognizer:tap];
        [self.scanImg addSubview:fullScrenButton];
        [self.scanImg addSubview:belowImageView];       //加背景图
    ...
    //  回放进度条
    {
        ...
        [bottomSlider setThumbImage:  [UIImage imageNamed:@"底部滑块_滑块.png"] forState:UIControlStateHighlighted];
    ...
    //  增益进度条
    {
        ...
        [sideSlider  setMaximumTrackImage:[UIImage imageNamed:@"右侧滑块_凹槽.png" ] forState:UIControlStateNormal];
        [sideSlider setThumbImage:[UIImage imageNamed:@"右侧滑块_滑块.png"] forState:UIControlStateHighlighted];
    ...
    m_imgArray = [NSMutableArray array];             //回放数据存储
...
-(void)fullScreenHiden{
    ...
    [fsbHiddenTimer invalidate];             //定时器停止
...
-(void)dataBaseHandle{
    ...
    NSString * srcPath = [[NSBundle mainBundle] pathForResource:@"modelData" ofType:@"db"];         //打开数据库，未有则创建
    NSLog(@"%@",srcPath);
    if (sqlite3_open([srcPath UTF8String], &db)!=SQLITE_OK) {
       ...
       [self creatTable :db];
        ...
        //然后初始显示表数据
        NSString *sqlQuery = @"SELECT  * FROM  UTRASOUND_IMAGE";//未建立table表!//
        ...
        if (sqlite3_prepare_v2(db, [sqlQuery UTF8String], -1, &statement, NULL) == SQLITE_OK) {
            ...
            while (sqlite3_step(statement) == SQLITE_ROW) {
                ...
                NSInteger primary_id  =   sqlite3_column_int(statement,0);
                
                int ss                =   sqlite3_column_bytes(statement, 1);
                
                Byte *bt              =   (Byte*)sqlite3_column_blob(statement, 2);
...
-(void) creatTable :(sqlite3*) database {

    char *errorMsg;
    
    const char *createSQL = "CREATE TABLE IF NOT EXISTS UTRASOUND_IMAGE (ID INTEGER PRIMARY KEY AUTOINCREMENT, BYTE_NUM  BYTES, IMAGE_DATA BLOB)";
...
-(void)clickFullScreen:(UIButton*)btn
 {              //全屏按钮
    ...
    if (btn.tag == 0)
    {
        ...
        self.scanImg.frame   = self.view.frame;          //放大到全屏
        
        belowImageView.frame = CGRectMake(0,0,self.scanImg.frame.size.width,self.scanImg.frame.size.height);                             //背影
...
-(void)slidingSideBoard:(UISlider*)sli{           //增益滑块
    //  关闭自动锁屏功能
...
//划块响应事件
-(void)sliderValueChange:(UISlider*)sli{            //回放滑块
    //  关闭自动锁屏功能
    [self disableAutoLock];
...
//图片播放功能
-(IBAction)playControl:(UIButton*)btn{            //回放控制
    //  关闭自动锁屏功能
    [self disableAutoLock];
...
- (IBAction)clickFreezing:(UIButton*)btn  {         //冻结，重启
    //  关闭自动锁屏功能
    [self disableAutoLock];
...
- (IBAction)clickZoomLeft:(id)sender {         // not  used
...
- (IBAction)clickZoomRight:(id)sender {       // not  used
...
-(void)selectChannel:(NSInteger)channel
  {
    ...
    [asyncSocketStateControl writeData:data withTimeout:-1 tag:1];     //发送状态 1
...
-(void)onSocket:(AsyncSocket *)sock didReadData:(NSData *)data withTag:(long)tag{
    ...
    switch (tag) {
    case 2:                                             // asycSocket  传原始数据
        {
            //  原始数据通道
            [imgData appendData:data];                 // socket to ctran
            ...
            if (imgData.length > 512) {
                ...
                if (bSuc && m_bRunning) {
                    ...
                    if( m_nSndRunning != 0) {
                        ...
                        [self ShowImageToWindow: pData];    //绘制图像，并缓存
                    }
            ...
            [sock readDataWithTimeout:-1 tag:2];       //加载下一个扫描线数据
            
        }
        break;
            
    case 3:                                      // asyncsocketreceivestate  传接收状态 3
        {
                                                //  状态通道
...
-(void)ShowImageToWindow: (NSData*) pData
{
    ...
    [self drawImage:newgotimg runing:m_bRunning];     //绘制图像
    
    
    [m_imgArray addObject:newgotimg];                //添加到缓存数组
...
-(void)ChangeBelowView:(double)scale{
    ...
    if ([UIScreen mainScreen].bounds.size.width==480) {      //height  ??//
...
- (IBAction)clickSettingButton:(id)sender {
    ...
    //无线信道
   // UILabel* wireless   =   [[UILabel alloc] initWithFrame:CGRectMake(20,120,200,20 )];
   // wireless.text       =    NSLocalizedString(@"Wireless Channel", nil) ;
   // [self.settingView  addSubview:wireless];
...
-(void)drawImage:(RawImag*)raw runing:(BOOL)running{
    ...
    if (raw.rawData == nil) {
        ...
        memcpy(drawImg,pDscor->DSCImage(rawdata),640*480*4);     //???   32 位
    ...
    //if (raw.zoom != nPrevZoom)
    {
        ...
        [self ChangeBelowView:scale];           //背景尺寸
    }
    ...
    labelDepth.text = [NSString stringWithFormat:@"%@: %@",NSLocalizedString(@"Depth_altrasound", nil),tmpdepth ];
    
    
    //  更新Gain
    
    int nshowgain = (int)((float)raw.gain * (105.0-30.0)/127.0 + 30.0); // y = 30+(105-30)*i/127
...
-(void) adjustItemStatus
{
    ...
    if (m_bConnected && m_bRunning) {
        sideSlider.enabled = YES;           //增益滑块
    } else {
        sideSlider.enabled = NO;
    }
    
    if (m_bRunning) {
        self.btnWifi.enabled = NO;         //wifi 开关
    } else {
        self.btnWifi.enabled = YES;
    }
    
    if (m_bRunning || m_imgArray==nil || m_imgArray.count<=0) {
        bottomSlider.enabled = NO;         //回放进度条
    } else {
        bottomSlider.enabled = YES;
    }
    
    if (m_imgArray==nil || m_imgArray.count<=0 || m_bRunning) {
        btnPre.enabled = NO;               //上一张图
        btnNext.enabled = NO;             //下一张图
        playButton.enabled = NO;         // 播放
    } else {
        btnPre.enabled = YES;
        btnNext.enabled = YES;
    ...
    if (!m_bRunning && m_bHaveImage && !m_bLoadedImage) {    //回放
        self.btnSave.enabled = YES;       //保存图像
    } else {
        self.btnSave.enabled = NO;
    }
    
    if (!m_bRunning && !m_bInCineLoop) {
        self.btnScan.enabled = YES;       //扫描
    } else {
        self.btnScan.enabled = NO;
    }
    
    if (!m_bRunning && !m_bInCineLoop) {
        self.btnSet.enabled = YES;      //通道选择
    } else {
...
-(void) nullImageRefresh
{
    if (!m_bHaveImage)
     ...
     {
         [nullImgTimer invalidate];     //消掉repeat
    }
}
